...
上游 innoextract 依赖安装包内嵌的版本字符串来选择二进制头部布局。有些安装包——尤其是伪装成正规
软件的恶意样本——会**伪造这个字符串**来对抗解包工具：它们保留一个看似可信但错误的版本号（例如声称
是 `6.2.1`，而真实格式其实是 `6.4.0.1`），并去掉 `(u)` 这个 Unicode 标记，于是任何版本的 innoextract
都会套用错误的布局，最终以 `Stream error while parsing setup headers!` 报错退出。

本分支在版本字符串无法识别时，改为**基于内容**来判定格式：它会逐个尝试所有已知的数据格式（从最新
开始），版本号和 Unicode 变体都取自格式定义本身、而非不可信的头部，并选用第一个能干净解析的布局。
被声明的版本号仍会作为警告打印出来，因此伪造行为是可见的、而不是致命的。正常的、版本已知的安装包
不受影响，仍走原有代码路径。

这对恶意软件分析和数字取证很有用——蓄意混淆的 Inno Setup 安装包在这些场景中很常见。

### 应急响应相关选项

本分支新增了若干选项，用于在**完全不运行**安装包的前提下对恶意 Inno Setup 安装包做快速研判：

* `--dump-code` —— 将编译后的 `[Code]`（Pascal 脚本）字节码导出为文件。内嵌的密码、C2 地址、释放
  路径以及反分析逻辑通常都藏在这里；上游虽然解析了这一段，却把它丢弃了。
* `--auto-password` —— 自动恢复加密密码：把 `[Code]` 脚本里所有字符串常量逐个拿去和安装包存储的
  密钥校验值比对。很多安装包加密载荷只是为了阻挡解包工具，却把密码写死在自己的脚本里，因此这一选项
  往往无需任何人工操作即可完成解密。
* `--report FILE` —— 输出一份 JSON 报告（应用标识、真实版本 vs. 声明版本、加密状态与恢复出的密码、
  释放文件路径及大小、`[Run]` 执行项），便于接入自动化研判流水线。
